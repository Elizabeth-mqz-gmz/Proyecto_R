...
Cargar datos:
```{r}
rawMxData <- read.csv('source/201126COVID19MEXICO.csv')
```
...
reLin <- glm(formula = Muertos~Obesos+Edad+Diabetes+Neumonia, coFrame, family = binomial(link = 'logit'))
summary(reLin)
```
...
```{r}
#punto de corte maximisa la calsificación de las observaciones
...
```{r}
#un 74% de las observaciones es muy buen resultado
Obs_correctas
```
